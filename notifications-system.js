...
class NotificationSystem {
    ...
    findNotificationBtn() {
        ...
        for (var i = 0; i < spans.length; i++) {
            if (spans[i].textContent.trim() === 'notifications') {
        ...
        dropdown.innerHTML =
            '<div class="p-3 border-b border-stone-200 dark:border-stone-700">' +
                '<div class="flex justify-between items-center">' +
                    '<h3 class="text-sm font-bold text-on-surface">Notifiche</h3>' +
                    '<button id="notif-mark-all" class="text-xs text-primary hover:text-primary/80">Segna tutte come lette</button>' +
                '</div>' +
            '</div>' +
        ...
        if (this.notifications.length === 0) {
            list.innerHTML =
                '<div class="p-6 text-center text-on-surface-variant">' +
                    '<span class="material-symbols-outlined text-3xl mb-2 block">notifications_none</span>' +
            ...
            for (var i = 0; i < this.notifications.length; i++) {
                ...
                html +=
                    '<div class="notification-item p-3 border-b border-stone-100 dark:border-stone-800 cursor-pointer hover:bg-stone-50 dark:hover:bg-stone-800 ' + (!n.is_read ? 'bg-primary/5' : '') + '" data-id="' + n.id + '">' +
                        '<div class="flex justify-between items-start">' +
                            '<div class="flex-1 min-w-0">' +
                                '<h4 class="text-sm ' + (!n.is_read ? 'font-bold' : 'font-medium') + ' text-on-surface">' + (n.title || '') + '</h4>' +
                                '<p class="text-xs text-on-surface-variant mt-0.5">' + (n.message || '') + '</p>' +
                                '<p class="text-xs text-stone-400 mt-1.5">' + this.formatDate(n.created_at) + '</p>' +
                            '</div>' +
                            (!n.is_read ? '<span class="w-2 h-2 bg-primary rounded-full mt-1 flex-shrink-0"></span>' : '') +
                        '</div>' +
                    '</div>';
            }
            list.innerHTML = html;

            // Aggiungi event listener per markAsRead
    ...
    async markAsRead(id) {
        try {
            ...
            console.error('[Notifiche] Errore markAsRead:', err.message);
        }
    }
    ...
    async markAllAsRead() {
        try {
            ...
            console.error('[Notifiche] Errore markAllAsRead:', err.message);
        }
    }
    ...
    showToast(title, message) {
        ...
        toast.innerHTML =
            '<div class="flex items-start gap-3">' +
                '<span class="material-symbols-outlined text-lg flex-shrink-0">notifications</span>' +
                '<div>' +
    ...
    async create(title, message, userId) {
        try {
            if (!window.supabase) return;
            var targetUserId = userId || this.userId;
            ...
            console.error('[Notifiche] Errore creazione:', err.message);
        }
    }
}
...
function initNotificationSystem() {
    if (window.supabase && typeof window.supabase.from === 'function') {
        ...
        setTimeout(initNotificationSystem, 200);
    }
}
