...
async function loadSession(req, res) {
    ...
    const admin = createClient(CONFIG.SUPABASE.URL, CONFIG.SUPABASE.SERVICE_ROLE_KEY, {
    ...
    const anon = createClient(CONFIG.SUPABASE.URL, CONFIG.SUPABASE.ANON_KEY, {
        auth: { persistSession: false, autoRefreshToken: false }
    });
...
async function loadIdentity(req, res) {
    ...
    const { data: operator } = await admin
        ...
        .select('id, user_id, email, ruolo, ruolo_portale, accesso_attivo, nome, cognome, permessi_dettagli, progetti_assegnati')
    ...
    if (portalRole) {
        ...
        role = legacyRole === 'admin' ? 'admin' : (legacyRole === 'senior' ? 'senior' : 'collaboratore');
    }
...
function pageAllowed(identity, pageKey) {
    ...
    if (pageKey === 'dashboard') {
        return identity.role === 'segretaria' || identity.role === 'collaboratore';
    }

    if (pageKey === 'dashboard_collaboratore') {
        return identity.role === 'collaboratore';
    }

    if (identity.role === 'segretaria') {
        return ['clienti', 'finanze', 'documenti'].includes(pageKey);
    }
...
express.static = function guardedStatic(root, options) {
    ...
    return (req, res, next) => guardPage(req, res, () => serve(req, res, next));
};
...
express.response.sendFile = function guardedSendFile(filePath, options, callback) {
    ...
    if (pageKey) {
        return guardPage(this.req || {}, this, () => originalSendFile.call(this, filePath, options, callback));
    }

    // Inject a small session-cookie synchronizer into the login page.
    if (normalized.endsWith('/templates/login.html')) {
        const res = this;
        fs.readFile(filePath, 'utf8').then((html) => {
            const syncScript = `<script>(function(){function set(name,value,maxAge){document.cookie=name+'='+encodeURIComponent(value)+'; Max-Age='+maxAge+'; Path=/; Secure; SameSite=Lax';}function clear(name){document.cookie=name+'=; Max-Age=0; Path=/; Secure; SameSite=Lax';}function bind(){try{if(typeof supabaseClient==='undefined'){setTimeout(bind,50);return;}supabaseClient.auth.getSession().then(function(r){if(r.data&&r.data.session){set('hdl_token',r.data.session.access_token,Math.floor((r.data.session.expires_at*1000-Date.now())/1000));set('hdl_refresh',r.data.session.refresh_token,2592000);}});supabaseClient.auth.onAuthStateChange(function(event,session){if(session){set('hdl_token',session.access_token,Math.floor((session.expires_at*1000-Date.now())/1000));set('hdl_refresh',session.refresh_token,2592000);}else if(event==='SIGNED_OUT'){clear('hdl_token');clear('hdl_refresh');}});}catch(e){setTimeout(bind,100);}}bind();})();</script>`;
            const output = html.replace('</body>', syncScript + '</body>');
