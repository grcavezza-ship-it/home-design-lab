-- Complete Database Setup for Home Design Lab
...
CREATE TABLE IF NOT EXISTS profiles (
    ...
    full_name TEXT,
    role TEXT DEFAULT 'client' CHECK (role IN ('client', 'architect', 'admin')),
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        ...
        VALUES (
            COALESCE(NEW.user_id, NEW.created_by, current_setting('app.current_user_id', true)::UUID),
            'CREATE',
            TG_TABLE_NAME,
            NEW.id,
            row_to_json(NEW)
    ...
    ELSIF TG_OP = 'UPDATE' THEN
        INSERT INTO activities (user_id, action, entity_type, entity_id, details)
        VALUES (
            COALESCE(NEW.user_id, NEW.assigned_to, current_setting('app.current_user_id', true)::UUID),
            'UPDATE',
            TG_TABLE_NAME,
            NEW.id,
...
CREATE TRIGGER log_projects_activities AFTER INSERT OR UPDATE OR DELETE ON projects
    FOR EACH ROW EXECUTE FUNCTION log_activity();

CREATE TRIGGER log_documents_activities AFTER INSERT OR UPDATE OR DELETE ON documents
    FOR EACH ROW EXECUTE FUNCTION log_activity();

CREATE TRIGGER log_messages_activities AFTER INSERT OR UPDATE ON messages
    FOR EACH ROW EXECUTE FUNCTION log_activity();

CREATE TRIGGER log_tasks_activities AFTER INSERT OR UPDATE OR DELETE ON tasks
    FOR EACH ROW EXECUTE FUNCTION log_activity();
...
-- Insert sample admin user (run this only if you don't have users yet)
-- INSERT INTO auth.users (id, email, email_confirmed_at, phone, raw_user_meta_data, created_at)
-- VALUES (
--     gen_random_uuid(),
--     'admin@homedesignlab.it',
--     now(),
--     NULL,
...
-- Sample project (run after creating users)
...
DO $$
BEGIN
    RAISE NOTICE 'Home Design Lab database setup completed successfully!';
    RAISE NOTICE 'Tables created: profiles, projects, documents, messages, activities, folders, tasks, notifications, permissions';
    RAISE NOTICE 'Indexes, triggers, and RLS policies have been applied';
    RAISE NOTICE 'Ready for use with Home Design Lab application';
END $$;
