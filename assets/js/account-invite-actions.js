(function () {
  ...
  const pages = {
    'gestione-team.html': 'collaboratore',
    'gestione-clienti.html': 'cliente'
  };
  ...
  function install() {
    ...
    button.type = 'button';
    button.className = 'hidden items-center gap-2 h-10 px-3 rounded-lg border border-stone-200 bg-white text-stone-600 hover:bg-stone-50 text-sm font-body not-italic shadow-sm';
    ...
    if (avatar && avatar.parentElement === container) container.insertBefore(button, avatar);
  ...
  function notify(message, success) {
    ...
    if (!el) {
      ...
      el.id = 'account-invite-notice';
      el.className = 'fixed top-24 right-5 z-[9999] max-w-sm rounded-xl border px-4 py-3 text-sm shadow-xl';
